...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    if (currentLocation != nil  && currentBarcodeID != -1) {
        Debug(@"Current barcodeID:%d",currentBarcodeID);
        NSNumber *doubleLongitude = [NSNumber numberWithDouble:currentLocation.coordinate.longitude];
        NSNumber *doubleLattitude = [NSNumber numberWithDouble:currentLocation.coordinate.latitude];
        NSString *longitudeString = [doubleLongitude stringValue];
        NSString *lattitudeString = [doubleLattitude stringValue];
        Debug(@"longitude:%@",longitudeString);
        Debug(@"lattitude:%@",lattitudeString);
        //insert junk into db
         agilitechAppDelegate *logisticsApp = (agilitechAppDelegate *) [[UIApplication sharedApplication] delegate];
        int locationID = [logisticsApp.sqliteController addInventoryToDatabase:currentBarcodeID withLatitude:lattitudeString andLongitude:longitudeString andName:name];
...
- (void) showBarcodeAlert:(Barcode *)barcode{
    dispatch_async( dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        agilitechAppDelegate *logisticsApp = (agilitechAppDelegate *) [[UIApplication sharedApplication] delegate];
        int barcodeID = [logisticsApp.sqliteController addBarCodeToDatabase:barcode];
...
//        alertMessage = [alertMessage stringByAppendingString:[barcode getBarcodeData]];
        alertMessage = @"Please enter description of item";
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    Debug(@"Entered: %@",[[alertView textFieldAtIndex:0] text]);
    if(buttonIndex == 0){
        //Code for Done button
        // TODO: Create a finished view
        [self getCurrentLocation:nil];
        name= [[NSString alloc] initWithString:[[alertView textFieldAtIndex:0] text]];
    }
    if(buttonIndex == 1){
